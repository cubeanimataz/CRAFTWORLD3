...
if !file.Exists(dir .. "/cfg.txt", "DATA") then
	file.Write(dir .. "/cfg.txt", util.TableToJSON(
	{
		...
		["fonts"] =
		{
			["regular"] = "AdLib Win95BT",
			["large"] = "Burbank Big Regular",
...
hook.Add( "Think", "Think_Lights!", function()
	...
	for k, c in pairs(ents.FindInSphere(LocalPlayer():GetPos(), proximity)) do
		if c:GetClass() == "ses_chest" && !IsValid(c:GetParent()) then
	...
	for k, d in pairs(ents.FindByClass("ses_chest")) do
...
if PickupInformed == nil then PickupInformed = false end
	...
	if IsValid(eyetrace) then
		...
		if eyetrace:GetClass() == "ses_pickup" then
		...
		elseif eyetrace:GetClass() == "ses_container" then
	...
	for k, pickup in pairs(ents.FindByClass("ses_pickup")) do
	...
	for _, npc in ipairs(ents.GetAll()) do
		...
		draw.NixieText( npc:GetNWString("botprefix", "") .. (npc.Name or (language.GetPhrase(npc:GetClass()))), "size95",npc.InfoPos.x,npc.InfoPos.y,  Color(255, 255, 255, transparency),TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER,2,Color(0, 0, 80, transparency/2),0, true)
		...
		draw.NixieText( npc:GetNWString("botgold","???") .. " Gold", "size65l",npc.InfoPos.x,npc.InfoPos.y-35,  Color(255, 255, 0, transparency),TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER,2,Color(80, 80, 0, transparency/2),0, true)

	end
	
	for k, v in pairs(ents.GetAll()) do
		if v:GetClass() == "ses_powerup" then
			if !v.AuraSound then
	...
	for k, v in pairs(player.GetAll()) do
		...
		if !v:IsBot() then
			if v:SteamID() == "STEAM_0:1:45185922" then
				local halo = ClientsideModel("models/ianmata1998/kingdomhearts2/crown_gold.mdl")
					...
					halo:Spawn()
					halo:SetPos(boneloc + Vector(0, math.sin(CurTime()*2), (25*v:GetModelScale()) + ((v:GetNWInt("deadstige", 0)+v:GetNWInt("prestige", 0)) * 3)))
		...
		if v:Alive() && v != LocalPlayer() && (LocalPlayer():GetEyeTrace().Entity == v or LocalPlayer().OverheadInfoP == "always" or LocalPlayer().OverheadInfoP == "minimal") && LocalPlayer().OverheadInfoP != "never" then
			if LocalPlayer().OverheadInfoP == "minimal" then
				...
				for a = 1, 10 do
					draw.SimpleTextOutlined(phrases[a] .. " " .. v:GetNWInt("cw3" .. data[a], "?"), "size65",pos.x,pos.y - 50 - (30*a), colours[a],1, 4, 2, Color(255,255,255))
				end
				...
				--else
					draw.SimpleTextOutlined( v:Nick(), "size95",pos.x,pos.y - 420, v:GetPlayerColor()*255,1, 4, 2, v:GetWeaponColor()*255)
				--end
			end
		end
	end
...
if IsValid(ply) then
	...
	if ply:GetNWString("heroname","Hankter") == "Geth" then
		ent:SetMaterial("null")	
	elseif IsValid(ply:GetActiveWeapon()) then
		if ply:GetActiveWeapon():GetClass() == "tfa_mmod_crowbar" or ply:GetActiveWeapon():GetClass() == "tfa_mmod_stunstick" then
			ent:SetMaterial("null")
		else
			ent:SetMaterial("")
		end
	end
	if ply:GetNWString("heroname","Hankter") == "Packmule" then
		if ply:GetActiveWeapon():GetClass() != "tfa_mmod_crowbar" and ply:GetActiveWeapon():GetClass() != "tfa_mmod_stunstick" then
			RunConsoleCommand("discardwep")
		end
	end
end
	...
	if ply:Alive() && ply:GetNWInt("healthstate", 0) > 0 && ply:GetActiveWeapon() != NULL then
		...
		-- end
		local element = ply:GetActiveWeapon():GetNWString("element", "normal")
		if element == "acid" then
			viewmodel:SetColor(Color(0,255,0))
		elseif element == "shock" then
			viewmodel:SetColor(Color(0,137,255))
		elseif element == "cryo" then
			viewmodel:SetColor(Color(117,255,255))
		elseif element == "explosive" then
			viewmodel:SetColor(Color(255,255,0))
		elseif element == "fire" then
			viewmodel:SetColor(Color(255,127,0))
		elseif element == "poison" then
			viewmodel:SetColor(Color(0,127,0))
		elseif element == "slag" then
			viewmodel:SetColor(Color(127,0,255))
		else
		...
		local weapon_name = ply:GetActiveWeapon():GetPrintName()
		if ply:GetActiveWeapon().NZPaPName && weaponrarity >= 15 then
			weapon_name = ply:GetActiveWeapon().NZPaPName
		end
		local textscale = textsize+25
		if string.len(weapon_name) > 16 then textscale = textscale-30 end
		surface.SetFont("size60")
		...
		draw.NixieText( weapon_name, "size60",ScrW()-10,ScrH()/1.35, color,2, 4,2,outlinecolor,0)
...
if !oldmaxhealth then
	oldmaxhealth = ply:GetNWInt("64maxhealth", 0)
	maxhp_burn = 0
	maxhp_cut = 0
end
...
maxhp_burn = math.max(0, maxhp_burn - 0.5)
maxhp_cut = math.max(0, maxhp_cut - 1.5)
...
if ply:GetNWInt("64armour",0) > 0 then
	hpw, hph = surface.GetTextSize(ply:GetNWString("64ap","???"))
end
local cornersize = 9
...
draw.NixieText("Zone " .. string.Comma(ply:GetNWInt("curzone","?")), "size200l", ScrW()/2, 45, Color(255,255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER,2,Color(0,0,0,255),0,true)
...
if ply:GetNWInt("crippled",0) == 1 then
	...
	if ply:GetNWInt("64armour",0) > 0 then
		draw.NixieText(ply:GetNWString("64ap","???"), "size" .. fontscale .. "l", 192+(math.Rand(-6,6)*hudpainshake_strength), ScrH()-50+(math.Rand(-6,6)*hudpainshake_strength), Color(150,150,150), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER,2,Color(85,85,85),0, true)
		draw.NixieText(ply:GetNWString("64hp","???") .. "/" .. ply:GetNWString("64mhp","???"), "size80l",192 + hpw+(math.Rand(-6,6)*hudpainshake_strength), ScrH()-50+(math.Rand(-6,6)*hudpainshake_strength), Color(70+(255*hudpainshake_strength),0,0), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER,2,Color(47+redness+yellowness+(255*hudpainshake_strength),yellowness,0),0, true)
	else
...
for i = 1, 3 do
	surface.SetDrawColor(Color(255,255,255))
	surface.SetMaterial(accountitems[math.Clamp(i,1,#accountitems)])
	surface.DrawTexturedRectRotated(ScrW()-30, 60*i, 60, 60, 0)
	draw.SimpleTextOutlined(ply:GetNWString("account_" .. accounttypes[math.Clamp(i,1,#accounttypes)], 0), "size110l", ScrW() - 60, 60*i, Color(255,255,255), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER, 2, Color(0,0,0))
end
if IsValid(ply) then
...
if levelup > 0 then
	levelupfade = math.min(levelupfade + 5,255)
	levelup = math.max(0,levelup-0.035)
else
	levelupfade = math.max(0,levelupfade-5)
end

if levelupfade > 0 then
	-- surface.SetMaterial(levelup_plate)
	-- surface.SetDrawColor(Color(255,255,255,levelupfade))
	-- if levelup > 0 then
		-- surface.DrawTexturedRectRotated(ScrW()/2,ScrH()/2,900,400,255-levelupfade)
	-- else
		-- surface.DrawTexturedRectRotated(ScrW()/2,ScrH()/2,900*(levelupfade/255) + (ScrW()*((255-levelupfade)/255)),400*(levelupfade/255),0)
	-- end
	-- for i = 1, 5 do
		-- draw.SimpleText( string.upper(phrase_LevelUp) .. "!", "size200",ScrW()/2 + ((65*(6-i))*(levelupfade/255)),ScrH()/2 + (math.sin(CurTime()*3-i)*10), Color(255,255 - ((255/8)*i),255 - ((255/8)*i),(levelupfade-30)/5*i),1, 1)
		-- draw.SimpleText( string.upper(phrase_LevelUp) .. "!", "size200",ScrW()/2 - ((65*(6-i))*(levelupfade/255)),ScrH()/2 + (math.sin(CurTime()*3-i)*10), Color(255,255 - ((255/8)*i),255 - ((255/8)*i),(levelupfade-30)/5*i),1, 1)
	-- end
	-- draw.SimpleText( string.upper(phrase_LevelUp) .. "!", "size200",ScrW()/2,ScrH()/2 + (math.sin(CurTime()*3)*10), Color(255,255,255,levelupfade),1, 1)
end

if !olderid then olderid = 0 end

if !erid then erid = 0 end

if olderid != ply:GetNWString("eridium",0) then
	erid = 500
	olderid = ply:GetNWString("eridium",0)
end

if erid > 0 then
	draw.NixieText( "[Gatholizum]" .. ply:GetNWString("eridium",0), "size" .. math.min(125,erid),ScrW()/2,ScrH()*0.75, Color(127,0,255,erid),1, 1,2,Color(190,125,255,erid),0)
	erid = math.max(0,erid-1)
end
...
for i = 1, ply:GetNWInt("deadstige",0) do
	surface.DrawTexturedRectRotated(94 + math.sin((CurTime()+i)*2)*5, ScrH()-yaxis-(20*i), 143.5 + math.sin((CurTime()-i)*2)*(redness/50), 51.5 - math.sin((CurTime()-i)*2)*(redness/50), math.sin(CurTime()-i*2)*(redness/50))
end
...
surface.DrawTexturedRectRotated(94 + math.sin((CurTime()+c+ply:GetNWInt("deadstige",0))*2)*5, ScrH()-yaxis-(20*c)-(ply:GetNWInt("deadstige",0)*20), 143.5 + math.sin((CurTime()-c)*2)*(redness/50), 51.5 - math.sin((CurTime()-c)*2)*(redness/50), math.sin(CurTime()-c*2)*(redness/50))
end

draw.NixieText( "CRAFTWORLD3 v2.9.5 BETA (c) Jen Walter 2020", "size55", ScrW(), 0, Color(199,199,255,122),2,0,0,Color(0,0,0,0), 0, true)
